...
/**
 * 类LoaderProperties.java的实现描述：Properties文件载入工具类. 可载入多个properties文件, 相同的属性在最后载入的文件中的值将会覆盖之前的值，但以System的Property优先.
 * @author coldface
 * @date   2016年5月30日下午6:18:03
 */
public class LoaderProperties {


    private static Logger logger = LoggerFactory.getLogger(LoaderProperties.class);

    private final Properties properties;

    public LoaderProperties(String... resourcesPaths) {
    ...
    private Properties loadProperties(String... resourcesPaths) {
        ...
        for (String location : resourcesPaths) {
            ...
            InputStream is = null;
            try {
                props.load(new InputStreamReader(LoaderProperties.class.getClassLoader().getResourceAsStream(location), "UTF-8"));
     ...
     * 载入多个文件, 文件路径使用Spring Resource格式.
     */
    public static Properties loadPropertiesStatic(String... resourcesPaths) {
        Properties props = new Properties();
